...
import { fetchBoardPulse } from '../../../lib/admin';
...
export function useLive({ hold, watching, onPulse }) {
  ...
  const eyes = React.useRef(watching);
  ...
  React.useEffect(() => {
    ...
    async function beat() {
      ...
      try {
        const pulse = await fetchBoardPulse(eyes.current);
        if (!alive) return;
        misses = 0;
        setState('live');
        setAt(Date.now());
        setHere((was) => {
          const found = pulse.here ?? [];
          return was.length === found.length && was.every((name, index) => name === found[index])
            ? was
            : found;
        });
        const mark = signature(pulse.boards ?? {});
        if (mark !== seen.current) {
          const first = seen.current === null;
          seen.current = mark;
          if (!first) hand.current?.(pulse.boards ?? {});
        }
      } catch {
        if (!alive) return;
        misses += 1;
