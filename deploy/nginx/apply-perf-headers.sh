...
if [ ! -f "$LINKS" ]; then
  say "Seeding $LINKS"
  cat > "$LINKS" <<'EOF'
# Generated by deploy/deploy-local.sh on every release: the Link header nginx
# sends on HTML responses, which is what Cloudflare turns into a 103 Early
# Hints response. The stylesheet is fingerprinted, so this file is rewritten
# with the release rather than written once — the same arrangement as the CSP
# style hash next to it, and it fails the same way if a release is published by
# moving the symlink instead of running the deploy script: the Link header then
# names a stylesheet that is no longer there and the browser wastes one request
# on a 404. Harmless, unlike the CSP hash, but still wrong.
#
# Two variables because the home page has no stylesheet to preload — it inlines
# its CSS — so `location = /` uses the font-only value and every other document
# uses the pair.
